...
export default async function AllProduct({params = {slug: []}}:{params?: {slug: string[]}}) {
  const query = groq`
  *[_type == "product"]{
    _id,
     rating,
  ...
  return (
    <div className="w-full max-w-[1920px] bg-[#F0F2F3]">
      <h1 className="font-bold p-5 text-5xl #64748B">All Products</h1>
        ...
        {response.map((items:any,index:number) => (
          <div className="max-w-[320px] bg-white/85 mb-2 p-5 rounded-xl" key={index}>
            ...
            <Link href={`/products/${items.slug.current}`} className="block">
              <button className="bg-[#029FAE] text-white duration-500 hover:bg-[#027A85] w-full py-1.5 rounded-lg">
                View Product
              </button>
            </Link>
          </div>
